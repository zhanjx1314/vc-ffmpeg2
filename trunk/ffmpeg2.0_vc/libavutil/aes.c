...
int av_aes_init(AVAES *a, const uint8_t *key, int key_bits, int decrypt)
{
    ...
    if (!enc_multbl[FF_ARRAY_ELEMS(enc_multbl)-1][FF_ARRAY_ELEMS(enc_multbl[0])-1]) {
		...
		{
		 const int cccc[4] = { 0xe, 0x9, 0xd, 0xb };
		 const int dddd[4] = { 0x2, 0x1, 0x1, 0x3 };
        init_multbl2(dec_multbl, cccc, log8, alog8, inv_sbox);
        init_multbl2(enc_multbl, dddd, log8, alog8, sbox);
		}
    }
