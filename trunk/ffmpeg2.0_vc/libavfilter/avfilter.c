    ...
    for (i = 0; i < filter->nb_inputs; i ++) {
        ...
        case AVLINK_UNINIT:
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (!link->sample_aspect_ratio.num && !link->sample_aspect_ratio.den)
				{
                    ...
                    link->sample_aspect_ratio = inlink ?
                        inlink->sample_aspect_ratio : avr;
				}
            ...
            case AVMEDIA_TYPE_AUDIO:
				...
				{
					AVRational avr = {1, link->sample_rate};
                if (!link->time_base.num && !link->time_base.den)
                    link->time_base = avr;
				}
            }
...
static const AVOption options[] = {
    { "thread_type", "Allowed thread types", OFFSET(thread_type), AV_OPT_TYPE_FLAGS,
        {  AVFILTER_THREAD_SLICE }, 0, INT_MAX, FLAGS, "thread_type" },
        { "slice", NULL, 0, AV_OPT_TYPE_CONST, {  AVFILTER_THREAD_SLICE },0, 0,0, "thread_type" },
    { "enable", "set enable expression", OFFSET(enable_str), AV_OPT_TYPE_STRING, {NULL},0,0, FLAGS },
    { NULL },
};
