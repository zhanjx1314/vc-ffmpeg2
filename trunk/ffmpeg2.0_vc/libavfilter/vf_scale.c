...
static int config_props(AVFilterLink *outlink)
{
    ...
    if (inlink->sample_aspect_ratio.num){
		 AVRational avr = {outlink->h * inlink->w, outlink->w * inlink->h};
        outlink->sample_aspect_ratio = av_mul_q(avr, inlink->sample_aspect_ratio);
    } else
...
static const AVOption scale_options[] = {
    ...
    { "flags", "Flags to pass to libswscale", OFFSET(flags_str), AV_OPT_TYPE_STRING, {"bilinear" }, FLAGS },
    { "interl", "set interlacing", OFFSET(interlaced), AV_OPT_TYPE_INT, {0 }, -1, 1, FLAGS },
    { "size",   "set video size",          OFFSET(size_str), AV_OPT_TYPE_STRING, {NULL}, 0, FLAGS },
    { "s",      "set video size",          OFFSET(size_str), AV_OPT_TYPE_STRING, {NULL}, 0, FLAGS },
    { NULL },
};
