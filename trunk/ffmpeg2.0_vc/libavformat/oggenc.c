...
static const AVOption options[] = {
    { "oggpagesize", "Set preferred Ogg page size.",
      offsetof(OGGContext, pref_size), AV_OPT_TYPE_INT, { 0}, 0, MAX_PAGE_SIZE, AV_OPT_FLAG_ENCODING_PARAM},
    { "pagesize", "preferred page size in bytes (deprecated)",
        OFFSET(pref_size), AV_OPT_TYPE_INT, {  0 }, 0, MAX_PAGE_SIZE, PARAM },
    { "page_duration", "preferred page duration, in microseconds",
        OFFSET(pref_duration), AV_OPT_TYPE_INT64, {  1000000 }, 0, INT64_MAX, PARAM },
    { NULL },
};
...
static int ogg_write_packet(AVFormatContext *s, AVPacket *pkt)
{
	...
	{
		 AVRational avr = { 1, st->codec->sample_rate };
        granule = pkt->pts + pkt->duration + av_rescale_q(st->codec->delay, avr, st->time_base);
	}else
