...
static int mov_read_chpl(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    for (i = 0; i < nb_chapters; i++) {
        ...
        str[str_len] = 0;
        avpriv_new_chapter(c->fc, i, avr, start, AV_NOPTS_VALUE, str);
    }
    return 0;
}
...
static int mov_read_dac3(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    st->codec->channels = achannels[acmod] + lfeon;
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
 ...
 retry:
    ...
    if (!sample) {
		 MOVAtom matom ={ AV_RL32("root"), INT64_MAX };
        mov->found_mdat = 0;
        if (!mov->next_root_atom)
            return AVERROR_EOF;
        avio_seek(s->pb, mov->next_root_atom, SEEK_SET);
        mov->next_root_atom = 0;
        if (mov_read_default(mov, s->pb, matom ) < 0 ||
            url_feof(s->pb))
            return AVERROR_EOF;
...
static const AVOption options[] = {
    {"use_absolute_path",
        ...
        offsetof(MOVContext, use_absolute_path), FF_OPT_TYPE_INT, { 0},
        0, 1, AV_OPT_FLAG_VIDEO_PARAM|AV_OPT_FLAG_DECODING_PARAM},
    {"ignore_editlist", "", offsetof(MOVContext, ignore_editlist), FF_OPT_TYPE_INT, { 0},
        0, 1, AV_OPT_FLAG_VIDEO_PARAM|AV_OPT_FLAG_DECODING_PARAM},
    {NULL}
};
