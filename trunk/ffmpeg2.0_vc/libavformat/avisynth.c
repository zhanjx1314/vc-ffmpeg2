...
static int avisynth_create_stream_video(AVFormatContext *s, AVStream *st) {
	...
	AVRational avr1= {avs->vi->fps_denominator, avs->vi->fps_numerator};
	AVRational avr2= {avs->vi->fps_numerator, avs->vi->fps_denominator};
...
static int avisynth_create_stream_audio(AVFormatContext *s, AVStream *st) {
    AviSynthContext *avs = s->priv_data;
	AVRational avr = {1, avs->vi->audio_samples_per_second};
...
static int avisynth_read_seek(AVFormatContext *s, int stream_index, int64_t timestamp, int flags) {
    AviSynthContext *avs = s->priv_data;
    AVStream *st;
    AVRational fps = {avs->vi->fps_numerator, avs->vi->fps_denominator};
	AVRational samplerate = {avs->vi->audio_samples_per_second, 1};
