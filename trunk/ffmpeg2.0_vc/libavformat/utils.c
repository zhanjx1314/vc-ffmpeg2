    ...
    while (size > 0 || (pkt == &flush_pkt && got_output)) {
        ...
        if (st->codec->codec_type == AVMEDIA_TYPE_AUDIO) {
            if (st->codec->sample_rate > 0) {
				  AVRational avr1 = { 1, st->codec->sample_rate };
                out_pkt.duration = av_rescale_q_rnd(st->parser->duration, avr1, st->time_base, AV_ROUND_DOWN);
            }
