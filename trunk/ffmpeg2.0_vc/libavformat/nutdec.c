...
static int decode_stream_header(NUTContext *nut)
{
    ...
    case 0:
		{
			const AVCodecTag * const aa_codec_tag[]= {ff_nut_video_tags, ff_codec_bmp_tags, 0};
        st->codec->codec_type = AVMEDIA_TYPE_VIDEO;
        st->codec->codec_id   = av_codec_get_id(aa_codec_tag, tmp);
		}
        break;
    case 1:
		{
			const AVCodecTag * const bb_codec_tag[] = {ff_nut_audio_tags,ff_codec_wav_tags,0};
        st->codec->codec_type = AVMEDIA_TYPE_AUDIO;
        st->codec->codec_id   = av_codec_get_id(bb_codec_tag,tmp);
		}
        break;
    case 2:
