...
static const AVOption options[] = {
    ...
    { "moov_size", "maximum moov size so it can be placed at the begin", offsetof(MOVMuxContext, reserved_moov_size), AV_OPT_TYPE_INT, { 0}, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, 0 },
    ...
    { "faststart", "Run a second pass to put the moov at the beginning of the file", 0, AV_OPT_TYPE_CONST, { FF_MOV_FLAG_FASTSTART}, INT_MIN, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM, "movflags" },
    FF_RTP_FLAG_OPTS(MOVMuxContext, rtp_flags),
    { "skip_iods", "Skip writing iods atom.", offsetof(MOVMuxContext, iods_skip), AV_OPT_TYPE_INT, { 1}, 0, 1, AV_OPT_FLAG_ENCODING_PARAM},
    ...
    { "use_editlist", "use edit list", offsetof(MOVMuxContext, use_editlist), AV_OPT_TYPE_INT, { -1}, -1, 1, AV_OPT_FLAG_ENCODING_PARAM},
    { "video_track_timescale", "set timescale of all video tracks", offsetof(MOVMuxContext, video_track_timescale), AV_OPT_TYPE_INT, { 0}, 0, INT_MAX, AV_OPT_FLAG_ENCODING_PARAM},
    { NULL },
};
...
static int mov_write_chpl_tag(AVIOContext *pb, AVFormatContext *s)
{
    ...
    for (i = 0; i < nb_chapters; i++) {
        ...
        AVDictionaryEntry *t;
		AVRational avr ={1,10000000};
        avio_wb64(pb, av_rescale_q(c->start, c->time_base, avr));
...
static void mov_create_chapter_track(AVFormatContext *s, int tracknum)
{
    ...
    for (i = 0; i < s->nb_chapters; i++) {
        AVChapter *c = s->chapters[i];
        AVDictionaryEntry *t;
        AVRational avr ={1,MOV_TIMESCALE};

        int64_t end = av_rescale_q(c->end, c->time_base, avr);
        pkt.pts = pkt.dts = av_rescale_q(c->start, c->time_base, avr);
...
const AVCodecTag* const mov_codec_tagxx[] = {ff_codec_movvideo_tags, ff_codec_movaudio_tags, 0};
AVOutputFormat ff_mov_muxer={
	...
	/*.interleave_packet=*/0,
	/*.query_codec=*/0,
	/*.get_output_timestamp=*/0,
};

#endif
#if CONFIG_TGP_MUXER
MOV_CLASS(tgp)
const AVCodecTag* const tgp_codec_tag[] = { codec_3gp_tags, 0 };
...
#if CONFIG_MP4_MUXER
MOV_CLASS(mp4)
const AVCodecTag* const mp4_codec_tag[] ={ ff_mp4_obj_type, 0 };
...
#if CONFIG_PSP_MUXER
MOV_CLASS(psp)
const AVCodecTag* const psp_codec_tag[] = { ff_mp4_obj_type, 0 };
...
#if CONFIG_TG2_MUXER
MOV_CLASS(tg2)
const AVCodecTag* const tg2_codec_tag[] = { codec_3gp_tags, 0 };
...
MOV_CLASS(ipod)
const AVCodecTag* const ipod_codec_tag[] = { codec_ipod_tags, 0 };
