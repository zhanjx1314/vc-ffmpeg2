...
static int mxf_parse_dv_frame(AVFormatContext *s, AVStream *st, AVPacket *pkt)
{
	...
	{
		AVRational avr1 = { 16, 9 };
		AVRational avr2 = { 4, 3 };
		if ((vs_pack[2] & 0x07) == 0x02)
			sc->aspect_ratio = avr1;
		else
			sc->aspect_ratio = avr2;
	}
    ...
    for(i = 0; i < pkt->size - 4; i++) {
        ...
        if (c == 0x1b5) {
            ...
            if (pkt->data[i+4]>>6 & 0x01) { // closed
			  ...
			  AVRational avr1 ={  4,  3};
			  AVRational avr2 = { 16,  9};
			  AVRational avr3 = {221,100};

			  e->flags |= 0x40;
			 switch ((pkt->data[i+4]>>4) & 0xf) {
            case 2:  sc->aspect_ratio = avr1; break;
            case 3:  sc->aspect_ratio = avr2; break;
            case 4:  sc->aspect_ratio = avr3; break;
            default:
...
static int mxf_write_header(AVFormatContext *s)
{
    ...
    if (!spf)
	{
		 AVRational avr1 = { 1, 25 };
        spf = ff_mxf_get_samples_per_frame(s, avr1);
	}
