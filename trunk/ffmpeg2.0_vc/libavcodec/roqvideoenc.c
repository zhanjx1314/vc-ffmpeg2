...
static void motion_search(RoqContext *enc, int blocksize)
{
    ...
    for (i=0; i<enc->height; i+=blocksize)
        for (j=0; j<enc->width; j+=blocksize) {
			motion_vect m = {{0,0}};
            lowestdiff = eval_motion_dist(enc, j, i, m, blocksize);
            bestpick.d[0] = 0;
            bestpick.d[1] = 0;
...
const AVRational avr[] = { {30,1}, {0,0}};
const enum AVPixelFormat roq_pix_fmts[] = {AV_PIX_FMT_YUV444P,AV_PIX_FMT_NONE };
...
AVCodec ff_roq_encoder = {
	...
	/*.id             =*/ AV_CODEC_ID_ROQ,
	/* capabilities = */ 0,
	/* supported_framerates = */ avr,
	/* pix_fmts = */ roq_pix_fmts,
