...
int ff_h263_decode_picture_header(MpegEncContext *s)
{
    ...
    */
    ...
    if (format != 7 && format != 6) {
        ...
        s->height = height;
		{
			AVRational avr1 = {12,11};
			AVRational avr2 = {1001, 30000};
			s->avctx->sample_aspect_ratio= avr1;
			s->avctx->time_base= avr2;
		}
    } else {
        int ufep;
        ...
        if (ufep) {
            if (format == 6) {
                ...
                */
                ...
                if (s->aspect_ratio_info == FF_ASPECT_EXTENDED) {
				  ...
				  AVRational avr = {12,11};
                width = ff_h263_format[format][0];
                height = ff_h263_format[format][1];
                s->avctx->sample_aspect_ratio= avr;
            }
            if ((width == 0) || (height == 0))
                return -1;
            s->width = width;
            ...
            if(s->custom_pcf){
                ...
                s->avctx->time_base.num /= gcd;
				  ...
				  AVRational avr = {1001, 30000};
                s->avctx->time_base= avr;
            }
        }
