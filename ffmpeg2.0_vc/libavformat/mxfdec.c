...
static int mxf_read_timecode_component(void *arg, AVIOContext *pb, int tag, int size, UID uid, int64_t klv_offset)
{
    ...
    case 0x1502:
		{
			AVRational avr = {avio_rb16(pb), 1};
			mxf_timecode->rate = avr;
		}
        break;
...
static int mxf_compute_sample_count(MXFContext *mxf, int stream_index, uint64_t *sample_count)
{
    ...
    if (!spf) {
		 AVRational avr = {mxf->current_edit_unit, 1};
        int remainder = (sample_rate.num * time_base.num) % (time_base.den * sample_rate.den);
        *sample_count = av_q2d(av_mul_q(avr, av_mul_q(sample_rate, time_base)));
        if (remainder)
            av_log(mxf->fc, AV_LOG_WARNING,
