...
static int dv_extract_video_info(DVDemuxContext *c, uint8_t* frame)
{
    ...
    if (c->sys) {
        ...
        avctx->bit_rate = av_rescale_q(c->sys->frame_size, avr,
                                       c->sys->time_base);
...
void ff_dv_offset_reset(DVDemuxContext *c, int64_t frame_offset)
{
    ...
    if (c->ach) {
        if (c->sys) {
		 AVRational avr = {8, c->ast[0]->codec->bit_rate};
        c->abytes= av_rescale_q(c->frames, c->sys->time_base, avr);
        }else
...
static int dv_read_header(AVFormatContext *s)
{
	...
	{
	AVRational avr = {8,1};
    s->bit_rate = av_rescale_q(c->dv_demux->sys->frame_size, avr,
