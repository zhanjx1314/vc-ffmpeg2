...
//      case AVMEDIA_TYPE_TEXT : ffio_wfourcc(pb, "txts"); break;
      ...
      if(stream->codec_type != AVMEDIA_TYPE_DATA){
        ...
        if(   stream->codec_type == AVMEDIA_TYPE_VIDEO
           ...
           AVRational avr = {stream->width, stream->height};

            int vprp= ff_start_tag(pb, "vprp");
            AVRational dar = av_mul_q(s->streams[i]->sample_aspect_ratio,avr);
            int num, den;
...
const AVCodecTag* const avi_codec_tag[] = {ff_codec_bmp_tags, ff_codec_wav_tags, 0};
