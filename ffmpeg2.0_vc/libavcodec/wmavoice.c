                       ...
                       int fcb_type,      int pitch)
{
    ...
    if (s->dc_level > 8) {
		...
		const float zero_coeffs[2] = { -1.99997,      1.0 };
		const float pole_coeffs[2] = { -1.9330735188, 0.93589198496 };
        ff_acelp_apply_order_2_transfer_function(samples, samples,
            zero_coeffs, pole_coeffs, 0.93980580475, s->dcf_mem, size);
    }
}
/**
 * @}
...
AVCodec ff_wmavoice_decoder = {
	/*.name           =*/ "wmavoice",
	/*.long_name      =*/ "Windows Media Audio Voice",
