        ...
        0)
    {
        ...
        //MPEG-2 fps
            ...
            if (s->aspect_ratio_info > 1) {
				  AVRational avr1 = {s1->pan_scan.width, s1->pan_scan.height};
				  AVRational avr2 = {s->width, s->height};
				  AVRational avr3 = {4, 3};
				  AVRational avr4 = {16, 9};
                AVRational dar =
                    av_mul_q(av_div_q(ff_mpeg2_aspect[s->aspect_ratio_info],avr1), avr2);

                // we ignore the spec here and guess a bit as reality does not match the spec, see for example
                // res_change_ffmpeg_aspect.ts and sequence-display-aspect.mpg
                // issue1613, 621, 562
                if ((s1->pan_scan.width == 0) || (s1->pan_scan.height == 0) ||
                   (av_cmp_q(dar, avr3 ) && av_cmp_q(dar, avr4))) {
					   AVRational avr6 = {s->width, s->height};
                    s->avctx->sample_aspect_ratio =
                        av_div_q(ff_mpeg2_aspect[s->aspect_ratio_info], avr6);
                } else {
					  AVRational avr5 = {s1->pan_scan.width, s1->pan_scan.height};
                    s->avctx->sample_aspect_ratio =
                        av_div_q(ff_mpeg2_aspect[s->aspect_ratio_info], avr5);
