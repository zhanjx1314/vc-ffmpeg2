...
static const AVOption buffer_options[] = {
    ...
    { "pix_fmt",       NULL,                     OFFSET(pix_fmt),          AV_OPT_TYPE_PIXEL_FMT,  {0},0,0, V },
...
#endif
    { "sar",           "sample aspect ratio",    OFFSET(pixel_aspect),     AV_OPT_TYPE_RATIONAL, {  1 }, 0, DBL_MAX, V },
    { "pixel_aspect",  "sample aspect ratio",    OFFSET(pixel_aspect),     AV_OPT_TYPE_RATIONAL, {  1 }, 0, DBL_MAX, V },
...
static av_cold int init_audio(AVFilterContext *ctx)
{
    ...
    if (!s->time_base.num)
	{
		AVRational avr = {1, s->sample_rate};
        s->time_base = avr;
	}
