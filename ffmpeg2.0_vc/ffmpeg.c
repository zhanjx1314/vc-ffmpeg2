                            ...
                            AVSubtitle *sub)
{
    ...
    for (i = 0; i < nb; i++) {
        ...
        // start_display_time is required to be 0
		{
			...
			sub->pts+= av_rescale_q(sub->start_display_time, avr , AV_TIME_BASE_Q);
		}
		...
		{
			AVRational avr = { 1, 1000 };
           pkt.duration = av_rescale_q(sub->end_display_time,avr , ost->st->time_base);
		}
        if (enc->codec_id == AV_CODEC_ID_DVB_SUBTITLE) {
...
static void do_streamcopy(InputStream *ist, OutputStream *ost, const AVPacket *pkt)
{
    ...
    if (ost->st->codec->codec_type == AVMEDIA_TYPE_AUDIO && pkt->dts != AV_NOPTS_VALUE) {
		 AVRational avr = {1, ist->st->codec->sample_rate};
        int duration = av_get_audio_frame_duration(ist->st->codec, pkt->size);
        if(!duration)
            duration = ist->st->codec->frame_size;
        opkt.dts = opkt.pts = av_rescale_delta(ist->st->time_base, pkt->dts, avr , duration, 
			&ist->filter_in_rescale_delta_last, ost->st->time_base) - ost_tb_start_time;
    }
...
static int decode_audio(InputStream *ist, AVPacket *pkt, int *got_output)
{
	...
	{
		 AVRational avr = {1, ist->st->codec->sample_rate};
        decoded_frame->pts = av_rescale_delta(decoded_frame_tb, decoded_frame->pts, avr, 
			decoded_frame->nb_samples, &ist->filter_in_rescale_delta_last, avr);
	}
    for (i = 0; i < ist->nb_filters; i++) {
        ...
        if (ost->stream_copy) {
            ...
            case AVMEDIA_TYPE_VIDEO:
                ...
                if (ost->frame_aspect_ratio.num) { // overridden by the -aspect cli option
                    AVRational avr ={ codec->height, codec->width }; 
					  sar =av_mul_q(ost->frame_aspect_ratio, avr);
            ...
            if (codec->codec_type == AVMEDIA_TYPE_VIDEO) {
                ...
                if (ist && !ost->frame_rate.num)
				{
                    ...
                    ost->frame_rate = ist->st->r_frame_rate.num ? ist->st->r_frame_rate : avr;
//                    ost->frame_rate = ist->st->avg_frame_rate.num ? ist->st->avg_frame_rate : (AVRational){25, 1};
            ...
            case AVMEDIA_TYPE_AUDIO:
                ...
                codec->channels       = avfilter_link_get_channels(ost->filter->filter->inputs[0]);
				{
					...
					AVRational avr = { 1, codec->sample_rate };
				   codec->time_base      = avr; 
				}
                break;
            case AVMEDIA_TYPE_VIDEO:
                ...
                codec->sample_aspect_ratio = ost->st->sample_aspect_ratio =
                    ...
                    av_mul_q(ost->frame_aspect_ratio, avr) :
...
static int process_input(int file_index)
{
    ...
    if (debug_ts) {
		 AVRational avrq = AV_TIME_BASE_Q;
        av_log(NULL, AV_LOG_INFO, "demuxer -> ist_index:%d type:%s "
               "next_dts:%s next_dts_time:%s next_pts:%s next_pts_time:%s pkt_pts:%s pkt_pts_time:%s pkt_dts:%s pkt_dts_time:%s off:%s off_time:%s\n",
               ifile->ist_index + pkt.stream_index, av_get_media_type_string(ist->st->codec->codec_type),
               av_ts2str(ist->next_dts), av_ts2timestr(ist->next_dts, &avrq),
               av_ts2str(ist->next_pts), av_ts2timestr(ist->next_pts, &avrq),
               av_ts2str(pkt.pts), av_ts2timestr(pkt.pts, &ist->st->time_base),
               av_ts2str(pkt.dts), av_ts2timestr(pkt.dts, &ist->st->time_base),
               av_ts2str(input_files[ist->file_index]->ts_offset),
               av_ts2timestr(input_files[ist->file_index]->ts_offset, &avrq));
    }
